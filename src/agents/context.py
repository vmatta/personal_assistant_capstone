...
_ALL_SCOPE_MARKERS = ("all scheduled", "all my", "all events", "all tasks", "everything", "clear all", "clear my")
_WEEKDAY_ORDER = ("monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday")


def resolve_delete_scope(user_text: str, reference_date: datetime) -> dict:
    """Deterministically resolve the scope of a delete/cleanup request into either
    specific target date(s) or "all open items", independent of any LLM call.

    Used in two places so they can never disagree:
      1. build_task_description (below) - to tell the CrewAI agent explicitly what
         "all" means when no day is named, instead of leaving it to guess (and
         silently default to "today").
      2. dispatch_node's post-action verification (src/graph/nodes.py) - to compute a
         ground-truth snapshot of which specific to-do IDs SHOULD be removed by this
         action, so the system can detect and self-heal if the agent's free-text
         "Deleted N task(s)" claim doesn't match what was actually persisted.

    Returns one of:
      {"kind": "day", "dates": ["2026-09-08", ...]}  - specific day(s) named
      {"kind": "all", "dates": []}                    - "all"/"everything", no day named
      {"kind": "unscoped", "dates": []}                - no day named and no "all" marker either
    """
    text = (user_text or "").lower()
    dates = []
    for day_name in _WEEKDAY_ORDER:
        if day_name in text:
            date_str = _get_next_weekday_date(day_name, reference_date)
            if date_str:
                dates.append(date_str)
    if dates:
        return {"kind": "day", "dates": sorted(set(dates))}
    if any(marker in text for marker in _ALL_SCOPE_MARKERS):
        return {"kind": "all", "dates": []}
    return {"kind": "unscoped", "dates": []}
...
def build_task_description(state: AssistantState, branch: ThoughtBranch) -> str:
    ...
    if branch.get("target_agent") == "scheduler_todo":
        ...
        if is_delete_action:
            ...
            # Determine the actual delete SCOPE from the user's own words, instead of leaving
            # the agent to guess. Without this, a request like "delete all scheduled events"
            # (no day name at all) had no explicit scope instruction — the agent would silently
            # default to interpreting it as "today", call list_todos, find nothing due for
            # today specifically, and report "0 tasks found to delete" even though many open
            # tasks exist on other days.
            #
            # resolve_delete_scope is also used by dispatch_node's post-action verification
            # (src/graph/nodes.py) so the two can never disagree about what "all" means.
            delete_scope = resolve_delete_scope(user_text, now)
            if delete_scope["kind"] == "day":
                scope_desc = " or ".join(delete_scope["dates"])
                scope_instruction = f"Scope: ONLY tasks due on {scope_desc}. Call list_todos, then filter to just that date before deleting."
            elif delete_scope["kind"] == "all":
                scope_instruction = (
                    "Scope: the user said 'all' with NO specific day named — this means EVERY open task "
                    "across ALL dates, not just today. Call list_todos (this returns ALL open tasks regardless "
                    "of date), then delete EVERY item returned, not just ones due today."
                )
            else:
                scope_instruction = (
                    "Scope: no specific day was named. Call list_todos to see what exists, then ask the user "
                    "to confirm whether they mean today's tasks or ALL tasks, UNLESS this is the "
                    "already-approved branch below, in which case treat it the same as 'all tasks'."
                )
            ...
            if already_approved:
                ...
                parts.append("1. Call list_todos to fetch tasks (returns ALL open tasks; filter by the Scope above if a specific day was named)")
                parts.append("2. Call complete_todo (or delete_note) for EACH matching task to actually delete it")
                parts.append("3. CONFIRM completion: 'Deleted N task(s) [for <day>/across all dates]: [list of what was deleted]'")
                parts.append("\n⚠️ CRITICAL RULES:")
                ...
                parts.append("- Do NOT silently assume 'today' when the user said 'all' with no day named — that means every open task.")
                parts.append("- If there are 0 matching tasks (after correctly applying the Scope above), say so plainly instead of asking a question.")
            else:
                parts.append("\n🗑️ DESTRUCTIVE ACTION DETECTED: User wants to clean up/delete tasks")
                parts.append(f"\n{scope_instruction}")
                parts.append("\nDELETE WORKFLOW (requires confirmation):")
                parts.append("1. FIRST: Call list_todos to fetch tasks (filter by the Scope above)")
                parts.append("2. SHOW the user what will be deleted: List all matching tasks with their times")
                parts.append("3. ASK FOR CONFIRMATION: 'I found X tasks [for <day>/across all dates]. Are you sure you want to delete ALL of them?'")
                parts.append("4. WAIT for explicit confirmation from user (yes/confirm/delete/proceed)")
                parts.append("5. Only if confirmed: Call complete_todo or delete_note for each matching task")
                parts.append("6. CONFIRM completion: 'Successfully cleaned up [N] tasks [for <day>/across all dates]'")
